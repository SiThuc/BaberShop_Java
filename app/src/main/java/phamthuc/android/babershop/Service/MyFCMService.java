package phamthuc.android.babershop.Service;
...
public class MyFCMService extends FirebaseMessagingService {
    ...
    private void updateLastBooking() {
        ...
        //Why we only work for current date? Because in my scenario, we load only
        //appointment for current date and next 3 days
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if(task.isSuccessful()){
                            if(task.getResult().size() > 0){
                                //Update
                                DocumentReference userBookingCuurentDocument = null;
                                for(DocumentSnapshot documentSnapshot:task.getResult()){
                                    userBookingCuurentDocument = userBooking.document(documentSnapshot.getId());
                                }
                                if(userBookingCuurentDocument != null){
                                    Map<String, Object> dataUpdate = new HashMap<>(  );
                                    dataUpdate.put("done", true);
                                    userBookingCuurentDocument.update( dataUpdate )
                                            .addOnFailureListener( new OnFailureListener() {
                                                @Override
                                                public void onFailure(@NonNull Exception e) {
                                                    Toast.makeText( MyFCMService.this, e.getMessage(), Toast.LENGTH_SHORT ).show();
